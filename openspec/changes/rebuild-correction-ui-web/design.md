...
- **Device auto-detect** — when `--device` is not set on the CLI (the common case), `post_infer` auto-selects `cuda:0` if `torch.cuda.is_available()` and `cpu` otherwise. The decision is printed to the terminal (`[infer] auto-selected device=…`) so it's diagnosable. This mirrors `DEVICE = 0 if torch.cuda.is_available() else 'cpu'` at the top of `test_column.ipynb` cell 2 — without the auto-detect the reviewer falls back to CPU even on machines with CUDA available, taking ~120 s per A0/300DPI plan instead of ~5 s on GPU. An explicit `--device cpu` still wins.
