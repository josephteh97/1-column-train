...
- **PRESERVED** `scripts/corrections_logger.py` — this file owns the on-disk schema that `scripts/retrain_yolo.py` and `scripts/hard_negative_pool.py` consume. Its READ surface (`new_job_id`, `iter_effective_corrections`, `summary`, plus `DB_PATH` / `JOBS_DIR` / `DATA_ROOT` constants) is preserved verbatim. The legacy WRITE helpers (`save_job`, `record_delete`, `record_edit`, `record_add`, `JobAlreadyCorrected`) are REMOVED in this change — see design.md "Single-writer storage policy" for why the FastAPI backend inlines the SQL into a single shared transaction per batch rather than calling those helpers. The schema and the JSON shape (the actual durable contract that downstream readers depend on) are unchanged.
