...
const state = {
  ...
  counts:        {unreviewed: 0, fp: 0, fn: 0, tp: 0},
};
...
function cachePalette() {
  ...
  state.palette = {
    ...
    tp:         css.getPropertyValue("--col-tp").trim(),
  };
}
  ...
  if (state.detections) {
    for (const det of state.detections) {
      ...
      switch (det.state) {
        ...
        case "TP":       color = state.palette.tp; break;
...
function refreshCounts() {
  state.counts = {unreviewed: 0, fp: 0, fn: 0, tp: 0};
  if (state.detections) {
    for (const det of state.detections) {
      switch (det.state) {
        case "FP":       state.counts.fp++; break;
        case "FN_ADDED": state.counts.fn++; break;
        case "TP":       state.counts.tp++; break;
        case "REMOVED":  break;
  ...
  document.getElementById("count-tp").textContent         = state.counts.tp;
}
...
function onCanvasDrag(ev) {
  ...
  if (state.pressState.startedOnDetection) return; // OSD's drag-on-detection is a no-op for us
  const imgPt = state.osd.viewport.viewerElementToImageCoordinates(ev.position);
...
function onKeyDown(e) {
  ...
  if (e.code === "Space") {
    if (!state.spaceHeld) {
      ...
      if (state.osd) state.osd.gestureSettingsMouse.dragToPan = true;
    }
    e.preventDefault();
    return;
  }
...
function onKeyUp(e) {
  if (e.code === "Space") {
    ...
    if (state.osd) state.osd.gestureSettingsMouse.dragToPan = false;
  }
}
...
async function postMark(payload) {
  ...
  const body = {job_id: state.jobId, ...payload};
  try {
    ...
    const data = await resp.json();
    if (payload.action === "FN_ADDED") {
      // The columns list grew — refetch to pick up the new entry's
      // bbox so paintOverlay can render it.
      await refetchDetections();
    } else {
...
async function postUndo() {
  ...
  try {
    ...
    // Undo of FN_ADDED stays in the JSON columns list (state flips to
    // FP per backend); a clean refetch keeps the UI in sync without
    // having to special-case the column-count change here.
    await refetchDetections();
  } catch (e) {
...
function triggerSubmit() {
  ...
  const total = state.counts.fp + state.counts.fn + state.counts.tp;
  alert(
    `Autosave is already on — your ${total} corrections ` +
    `(${state.counts.fp} FP, ${state.counts.fn} FN, ` +
    `${state.counts.tp} TP) are persisted in data/corrections.db.\n\n` +
